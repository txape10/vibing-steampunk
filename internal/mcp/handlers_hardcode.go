...
// either via direct SELECT (D010TAB) or via ZCL_GET_HARDCODE accessor
// (WBCROSSGT OTYPE='CLAS'). Returns MEDIUM-confidence callers; grep promotion
...
func (s *Server) fetchHardcodeGlobalCallers(ctx context.Context) (map[hardcodeCallerKey]*graph.HardcodeCaller, error) {
	...
	// Path A: direct SELECT on ZTCA_HARDCODE via D010TAB (program→table cross-reference).
	// D010TAB.MASTER is the include/program name; TABNAME is the DB table.
	// This replaces CROSS (CROSS.TYPE is CHAR(1) — 'DA' is invalid).
	...
	if err != nil {
		return nil, fmt.Errorf("D010TAB query for ZTCA_HARDCODE: %w", err)
	}
	if crossResult != nil {
		for _, row := range crossResult.Rows {
			include := strings.TrimSpace(fmt.Sprintf("%v", row["MASTER"]))
			if include == "" {
				continue
			}
			// Skip SAP-internal generated includes. D010TAB records class-pool and
			// function-group sub-includes under names beginning with '~' (e.g.
			// ~ZCL_GET_HARDCODE===CP, ~ZVCA_HARDCODETOP===CP). These are not real
			// callable objects and should not appear as callers.
			if strings.HasPrefix(include, "~") {
				continue
			}
			// Skip SAP Workbench-generated table browser programs (/1BCDWB/DB*).
			// These are generated by SE16/SM30 for direct table maintenance and are
			// never real business callers.
			if strings.HasPrefix(include, "/1BCDWB/") {
				continue
			}
			// Skip the accessor class itself — it reads ZTCA_HARDCODE internally and
			// is not a business caller. Belt-and-suspenders: covers any remaining form
			// D010TAB might use to record it.
			if strings.Contains(strings.ToUpper(include), strings.ToUpper(hardcodeAccessorClass)) {
				continue
			}
	...
	// Path B: accessor class ZCL_GET_HARDCODE (WBCROSSGT OTYPE='CL' — 2-char type code for classes)
