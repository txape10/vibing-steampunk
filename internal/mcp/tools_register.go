...
func (s *Server) registerCRUDTools(shouldRegister func(string) bool) {
	...
	if shouldRegister("CreateDomain") {
		s.mcpServer.AddTool(mcp.NewTool("CreateDomain",
			mcp.WithDescription("Create a DDIC domain (SE11 DOMA). Handles full workflow: create → write XML metadata → activate. Domains define the technical type and optional fixed-value list shared across data elements."),
			mcp.WithString("name",
				mcp.Required(),
				mcp.Description("Domain name (uppercase, max 30 chars, Z/Y prefix)"),
			),
			mcp.WithString("description",
				mcp.Required(),
				mcp.Description("Short description"),
			),
			mcp.WithString("data_type",
				mcp.Required(),
				mcp.Description("Base ABAP type: CHAR, NUMC, INT4, INT2, DEC, CURR, QUAN, DATS, TIMS, CLNT, LANG, ACCP…"),
			),
			mcp.WithNumber("length",
				mcp.Required(),
				mcp.Description("Field length (e.g. 10 for CHAR10)"),
			),
			mcp.WithNumber("decimals",
				mcp.Description("Decimal places (for DEC/CURR/QUAN, default 0)"),
			),
			mcp.WithBoolean("lowercase",
				mcp.Description("Allow lowercase input (default false)"),
			),
			mcp.WithString("fixed_values",
				mcp.Description(`JSON array of fixed values: [{"value":"A","description":"Active"},{"value":"I","description":"Inactive"}]`),
			),
			mcp.WithString("package",
				mcp.Description("Target package (default: $TMP)"),
			),
	...
	if shouldRegister("CreateDataElement") {
		s.mcpServer.AddTool(mcp.NewTool("CreateDataElement",
			mcp.WithDescription("Create a DDIC data element (SE11 DTEL). Handles full workflow: create → write XML metadata → activate. Data elements define field labels and reference a domain or built-in type."),
			mcp.WithString("name",
				mcp.Required(),
				mcp.Description("Data element name (uppercase, max 30 chars, Z/Y prefix)"),
			),
			mcp.WithString("description",
				mcp.Required(),
				mcp.Description("Short description"),
			...
			mcp.WithString("type_name",
				mcp.Required(),
				mcp.Description("Domain name (e.g. ZDOMA_STATUS) or ABAP built-in type (e.g. CHAR10, INT4). Used when type_kind is 'predefinedAbapType'."),
			),
			mcp.WithString("type_kind",
				mcp.Description("'domain' (default) or 'predefinedAbapType'"),
			),
			mcp.WithString("data_type",
				mcp.Description("Underlying ABAP type (CHAR, INT4, DATS…). Required when type_kind='domain': set to the domain's base type (e.g. 'CHAR'). For predefinedAbapType: auto-derived from type_name."),
			),
			mcp.WithNumber("data_type_length",
				mcp.Description("Underlying type length (e.g. 10 for CHAR10). Required when type_kind='domain'."),
			),
			mcp.WithNumber("data_type_decimals",
				mcp.Description("Underlying type decimals (default 0)."),
			),
			mcp.WithString("label_short",
				mcp.Description("Short field label (max 10 chars, defaults to description)"),
			),
			mcp.WithString("label_medium",
				mcp.Description("Medium field label (max 20 chars, defaults to description)"),
			),
			mcp.WithString("label_long",
				mcp.Description("Long field label (max 40 chars, defaults to description)"),
			),
			mcp.WithString("label_heading",
				mcp.Description("Column heading (max 55 chars, defaults to description)"),
			),
			mcp.WithString("search_help",
				mcp.Description("F4 search help name (optional)"),
			),
			mcp.WithString("parameter_id",
				mcp.Description("SET/GET parameter ID (optional)"),
			),
			mcp.WithString("package",
				mcp.Description("Target package (default: $TMP)"),
			),
	...
	if shouldRegister("CreateTableType") {
		s.mcpServer.AddTool(mcp.NewTool("CreateTableType",
			mcp.WithDescription("Create a DDIC table type (SE11 TTYP). Handles full workflow: create → write XML metadata → activate. Table types define the type of an internal table with a fixed row structure."),
			mcp.WithString("name",
				mcp.Required(),
				mcp.Description("Table type name (uppercase, max 30 chars, Z/Y prefix)"),
			),
			mcp.WithString("description",
				mcp.Required(),
				mcp.Description("Short description"),
			),
			mcp.WithString("row_type_name",
				mcp.Required(),
				mcp.Description("Row type: structure or table name (e.g. BAPIRET2, ZS_MY_STRUCT). Uppercase."),
			),
			mcp.WithString("row_type_kind",
				mcp.Description("'dictionaryType' (default, use DDIC structure/table) or 'predefinedAbapType'"),
			),
			mcp.WithString("access_type",
				mcp.Description("Table category: 'standard' (default), 'sorted', 'hashed'"),
			),
			mcp.WithString("key_definition",
				mcp.Description("Key definition: 'standard' (default), 'rowType', 'notSpecified'"),
			),
			mcp.WithString("key_kind",
				mcp.Description("Key uniqueness: 'nonUnique' (default), 'unique', 'notSpecified'"),
			),
			mcp.WithString("package",
				mcp.Description("Target package (default: $TMP)"),
			),
	...
	if shouldRegister("CreateLockObject") {
		s.mcpServer.AddTool(mcp.NewTool("CreateLockObject",
			mcp.WithDescription("Create a DDIC lock object (SE11 ENQU). Handles full workflow: create → write XML metadata → activate. SAP convention: lock object name starts with E (e.g. EZ_MY_TABLE). Activation generates ENQUEUE_/DEQUEUE_ function modules automatically."),
			mcp.WithString("name",
				mcp.Required(),
				mcp.Description("Lock object name (uppercase, max 30 chars, convention: E prefix, e.g. EZ_MY_TABLE)"),
			),
			mcp.WithString("description",
				mcp.Required(),
				mcp.Description("Short description"),
			),
			mcp.WithString("primary_table",
				mcp.Required(),
				mcp.Description("Primary table to lock (e.g. ZMY_TABLE)"),
			),
			mcp.WithString("lock_mode",
				mcp.Description("Lock mode: 'E' exclusive (default), 'S' shared, 'X' exclusive non-cumulative"),
			),
			mcp.WithString("lock_parameters",
				mcp.Description(`JSON array of key fields to expose as lock parameters: [{"table_name":"ZMY_TABLE","field_name":"BUKRS","wanted":true},{"table_name":"ZMY_TABLE","field_name":"BELNR","wanted":true}]`),
			),
			mcp.WithBoolean("allow_rfc",
				mcp.Description("Allow RFC calls (default false)"),
			),
			mcp.WithString("package",
				mcp.Description("Target package (default: $TMP)"),
			),
